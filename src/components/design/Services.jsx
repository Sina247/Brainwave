...
export const PhotoChatMessage = () => {
	return (
		<div className="absolute top-8 right-8 w-[18rem] h-[6rem] py-6 px-8 bg-black rounded-t-xl rounded-bl-xl font-code text-base lg:top-16 lg:right-[8.75rem]">
			<Typewriter
				options={{
					strings: ["Hey Brainwave, enhance this photo.", "Hey Brainwave, give this photo a boost.", "Brainwave, enhance the details in this photo.", "Activate Brainwave mode to refine this photo.", "Brainwave, apply your magic to this photo."],
					autoStart: true,
					loop: true,
				}}
			/>
...
export const VideoChatMessage = ({ isPlaying }) => {
	return (
		<div className="absolute top-8 left-[3.125rem] w-full max-w-[14rem] pt-2.5 pr-2.5 pb-7 pl-5 bg-n-6 rounded-t-xl rounded-br-xl font-code text-base md:max-w-[17.5rem]">
			...
			<div className="tagline absolute right-2.5 bottom-1 text-[0.625rem] text-n-3 uppercase">{isPlaying ? <img src={loading} alt="Loading" className="w-4 h-4 animate-spin pointer-events-none select-none" /> : <span>just now</span>}</div>
...
export const VideoBar = ({ isPlaying, setIsPlaying }) => {
	...
	return (
		<div className="absolute left-0 bottom-0 w-full flex items-center p-6">
			<img src={isPlaying ? pause : play} width={24} height={24} alt="Play" className="object-contain mr-3 cursor-pointer" onClick={() => setIsPlaying(!isPlaying)} />
